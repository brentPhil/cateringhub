...
describe("VirtualizedCombobox Performance", () => {
  it("should only render visible items in the DOM", () => {
    ...
    const performanceGain =
      ((totalItems - expectedRenderedItems) / totalItems) * 100;
  ...
  it("should handle large datasets efficiently", () => {
    ...
    datasets.forEach((dataset) => {
      ...
      const memoryReduction =
        ((dataset.count - maxRenderedItems) / dataset.count) * 100;

      expect(memoryReduction).toBeGreaterThan(95);
      console.log(
        `${dataset.name}: ${memoryReduction.toFixed(2)}% memory reduction`
      );
    });
  });
...
describe("Performance Benchmarks", () => {
  it("should demonstrate performance improvements", () => {
    ...
    const renderTimeImprovement =
      ((benchmarks.standardCombobox.initialRender -
        benchmarks.virtualizedCombobox.initialRender) /
        benchmarks.standardCombobox.initialRender) *
      100;

    const domNodeReduction =
      ((benchmarks.standardCombobox.domNodes -
        benchmarks.virtualizedCombobox.domNodes) /
        benchmarks.standardCombobox.domNodes) *
      100;

    const memoryReduction =
      ((benchmarks.standardCombobox.memoryUsage -
        benchmarks.virtualizedCombobox.memoryUsage) /
        benchmarks.standardCombobox.memoryUsage) *
      100;
